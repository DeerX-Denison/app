 ...
 */
const useItemDisplayTime = (oldTime: Date | undefined, curTime: Date) => {
					...
					if (curTime.getDate() === oldTime.getDate()) {
						if (curTime.getHours() === oldTime.getHours()) {
							if (curTime.getMinutes() === oldTime.getMinutes()) {
								...
								setDisplayTime('Now');
							} else if (curTime.getMinutes() - oldTime.getMinutes() < 30) {
								// same year, month, date, hour, < 30 minutes apart
								setDisplayTime(
									`${curTime.getMinutes() - oldTime.getMinutes()} minutes ago`
								);
							} else {
								// same year, month, date, hour, >= 30 minutes apart
...
export default useItemDisplayTime;
